    ...
    try {
      ...
      if (result.signal) {
        ...
        allSignals[symbol] = {
          ...
          vwapReversion: { name: 'VWAP Reversion', score: 0, confidence: 0, reason: 'No market data available' },
...
function combineSignals(symbol, trendSignals, reversionSignals, regime, learnedWeights = null) {
  ...
  return {
    ...
    vwapReversion: reversionSignals[1] || { name: 'VWAP Reversion', score: 0, confidence: 0, reason: 'N/A' },
...
async function main() {
  ...
  for (const holding of portfolio.holdings) {
    ...
    if (sellPercent > 0) {
      ...
      const trade = {
        ...
        total,
        reason: sellReason,
        signals: signal || { symbol: holding.symbol, timestamp: new Date().toISOString(), momentum: {}, meanReversion: {}, vwapReversion: {}, technical: {}, combined: 0, recommendation: 'HOLD' },
  ...
  for (const holding of dustHoldings) {
    ...
    const trade = {
      ...
      signals: dustSignal || { symbol: holding.symbol, timestamp: new Date().toISOString(), momentum: {}, meanReversion: {}, vwapReversion: {}, technical: {}, combined: 0, recommendation: 'HOLD' },
      entrySignals: holding.entrySignals || null,
